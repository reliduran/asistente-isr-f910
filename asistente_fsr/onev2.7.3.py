...
class SistemaNominaApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.state("zoomed")  # Se abre maximizado respetando la barra de tareas
        ...
        # 2. Barra Lateral (Sidebar) NUEVA ESTRUCTURA CON SCROLL ---
        side_outer = tk.Frame(container, bg=STYLE_CONFIG["bg_sidebar"], width=260)
        side_outer.grid(row=0, column=0, sticky="nsew")
        side_outer.grid_propagate(False)

        side_canvas = tk.Canvas(
            side_outer, bg=STYLE_CONFIG["bg_sidebar"], highlightthickness=0, width=260
        )
        side_scrollbar = ttk.Scrollbar(
            side_outer, orient="vertical", command=side_canvas.yview
        )

        # Este es el frame que contiene los botones. Se llama self.sidebar para no romper tu código.
        self.sidebar = tk.Frame(side_canvas, bg=STYLE_CONFIG["bg_sidebar"], width=260)

        self.sidebar.bind(
            "<Configure>",
            lambda e: side_canvas.configure(scrollregion=side_canvas.bbox("all")),
        )
        side_canvas.create_window((0, 0), window=self.sidebar, anchor="nw", width=260)
        side_canvas.configure(yscrollcommand=side_scrollbar.set)

        side_canvas.pack(side="left", fill="both", expand=True)
        side_scrollbar.pack(side="right", fill="y")

        def _on_side_wheel(event):
            side_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

        side_canvas.bind_all("<MouseWheel>", _on_side_wheel)
        # --- FIN DEL WRAPPER ---
...
class PageReportes(tk.Frame):
    def __init__(self, parent, controller):
        ...
        canvas = tk.Canvas(
            container, bg=STYLE_CONFIG["bg_content"], highlightthickness=0
        )
        scrollbar = ttk.Scrollbar(container, orient="vertical", command=canvas.yview)
        grid_frame = tk.Frame(canvas, bg=STYLE_CONFIG["bg_content"])
...
class PageManual(tk.Frame):
    def __init__(self, parent, controller):
        ...
        **SISTEMA DE GESTIÓN FISCAL F910 - GUÍA RÁPIDA - Version 2.7.3

        OBJETIVO GENERAL: Automatizar la consolidación, cálculo y validación de los ingresos anuales del personal
        para la presentación del Informe F910 ante el Ministerio de Hacienda. Esta version soporta monitores de baja resolución
        ...
        *   Resultado: Genera el "Informe_F910_Final.xlsx", listo para ser cargado en la plataforma de Hacienda.
        ...
        ---
        DESARROLLADOR: Ruben Eli Duran Ramirez | Versión: ISR/F910 V2.7.3 (Híbrida) | www.linkedin.com/in/reliduran/
        """
